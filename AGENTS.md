...
优先下载到临时目录 tmp（避免网络原因无法访问，需要最新规范再更新），再读取该入口及其引用的 specs/ 规范，并持续遵守其全部要求。

该入口为"分类与懒加载（加载调度器）"结构，按**必加载层 / 通用层 / 技术栈层 / 平台层**分层存储于 `specs/` 下，按需懒加载（涉及到相关内容时的必选项，必须加载）。文中 `specs/...` 均为**相对该入口文件所在目录**的路径，经 https 访问时解析为 `https://agent.c332030.com/specs/...`。
