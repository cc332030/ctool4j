 ...
 * <h2>执行方式</h2>
 * <ul>
 *   <li>性能测试独立于单元测试，仅在明确命令执行时才运行（{@code mvn -Pperf test -Dtest=CCollUtilsPerfTests}）。</li>
...
public class CCollUtilsPerfTests {
    ...
    /**
     * 基准执行入口（显式运行：mvn -Pperf test -Dtest=CCollUtilsPerfTests -DfailIfNoTests=false）
    ...
    private static class CCollToMapPredicateCase implements CBenchmarkCase {
        ...
        public Object run() {
            return CCollUtils.toMap(
                    items, Item::getId, Item::getName, id -> id != null, null);
        }
    }
