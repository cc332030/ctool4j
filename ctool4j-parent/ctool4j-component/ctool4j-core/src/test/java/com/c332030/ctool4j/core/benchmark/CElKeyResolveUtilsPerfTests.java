 ...
 * <h2>执行方式</h2>
 * <ul>
 *   <li>性能测试独立于单元测试，仅在明确命令执行时才运行（{@code mvn -Pperf test -Dtest=CElKeyResolveUtilsPerfTests}）。</li>
...
public class CElKeyResolveUtilsPerfTests {

    /**
     * 基准执行入口（显式运行：mvn -Pperf test -Dtest=CElKeyResolveUtilsPerfTests -DfailIfNoTests=false）
    ...
    private static Method method(String name) {
        for (Method m : CElKeyResolveUtilsPerfTests.class.getDeclaredMethods()) {
