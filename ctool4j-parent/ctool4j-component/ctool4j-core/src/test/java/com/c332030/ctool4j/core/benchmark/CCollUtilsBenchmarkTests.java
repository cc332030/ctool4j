 ...
 * <h2>执行方式</h2>
 * <ul>
 *   <li>性能测试独立于单元测试，仅在明确命令执行时才运行（{@code mvn -Pperf test -Dtest=CCollUtilsBenchmarkTests}）。</li>
...
public class CCollUtilsBenchmarkTests {
    ...
    /**
     * 基准执行入口（显式运行：mvn -Pperf test -Dtest=CCollUtilsBenchmarkTests -DfailIfNoTests=false）
