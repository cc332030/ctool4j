...
public class CHttpClientUtilsTests {
    ...
    public void requestConfig() {
        ...
        // HttpClient 5 的 RequestConfig 超时以 Timeout 表达，且 socket 超时改称 responseTimeout
        Assertions.assertEquals(CHttpClientUtils.CONNECTION_REQUEST_TIMEOUT,
                CHttpClientUtils.REQUEST_CONFIG.getConnectionRequestTimeout().toMilliseconds());
        Assertions.assertEquals(CHttpClientUtils.CONNECT_TIMEOUT,
                CHttpClientUtils.REQUEST_CONFIG.getConnectTimeout().toMilliseconds());
        Assertions.assertEquals(CHttpClientUtils.SOCKET_TIMEOUT,
                CHttpClientUtils.REQUEST_CONFIG.getResponseTimeout().toMilliseconds());
