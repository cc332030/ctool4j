...
public class CSecurityConfiguration {
    ...
    public SecurityFilterChain cFilterChain(
        ...
        val chain = http
            ...
            // 禁用自带的表单登录 /login
            //（Spring Security 7 起 formLogin 仅保留 formLogin(Customizer) 重载，无参重载已移除）
            .formLogin(AbstractHttpConfigurer::disable)
            // 禁止 anonymous
            //（Spring Security 7 同上：anonymous 仅保留 anonymous(Customizer) 重载）
            ...
            .authorizeHttpRequests(authorize -> authorize
                ...
                // 不需要认证的地址有哪些
                .requestMatchers(requestMatchersPathConfig.getPermits())
                .permitAll()

                // 禁止的地址有哪些
                .requestMatchers(requestMatchersPathConfig.getDenies())
                .denyAll()
