...
import com.c332030.ctool4j.web.exception.handler.CUnauthorizedExceptionHandler;
 ...
 * 且 HTTP 状态显式声明为 401。</p>
 *
 * <h2>设计思路</h2>
 * <ul>
 *   <li>直接实例化处理器调用 handle（与同目录其他处理器用例一致），不启容器。</li>
 *   <li>HTTP 状态通过 {@code @ResponseStatus} 注解断言固化：本模块其余处理器均为 HTTP 200，未授权刻意偏离为 401，需防回改。</li>
 * </ul>
 * <h2>覆盖场景与未覆盖</h2>
 * <ul>
 ...
 *   <li>1.3 HTTP 状态声明为 401（handle_declaresHttp401）</li>
 * </ul>
 *
 * @author c332030
 * @since 2026/9/17
...
class CUnauthorizedExceptionHandlerTests {
    ...
    /**
     * <p>对应测试用例 1.3：HTTP 状态显式声明为 401（本模块唯一偏离 HTTP 200 的处理器）</p>
     */
    @Test
    void handle_declaresHttp401() throws NoSuchMethodException {
        ...
        Assertions.assertNotNull(responseStatus, "必须显式声明 HTTP 状态：未授权应为 401（RFC 7235）");
