 ...
 * <h2>能力目录</h2>
 * <ul>
 *   <li>通过 {@code @ConditionalOnMissingExceptionHandler(IllegalStateException.class)} 控制：容器存在其他同类型处理器时本处理器不生效。</li>
...
public class CIllegalStateExceptionHandler {
     ...
     * 异常消息为 null（如 {@code new IllegalStateException()}）时用异常类型简单名兜底，避免 {@code message: null}；见类 javadoc「兜底设计」。</p>
     *
     * @param e 非法状态异常
     * @return 错误结果
     */
    ...
    public CStrResult<Void> handle(IllegalStateException e) {
        ...
        if (null == message) {
            return CStrResult.error(null == e ? "非法状态" : e.getClass().getSimpleName());
        }
