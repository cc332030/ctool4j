...
- **会话服务**：`CAbstractSessionService` 基于 Redis 提供会话存取、按 jwt 取会话、当前会话获取；当前会话缺失（未授权）时 `get()`/`check()` 抛 `CUnauthorizedException`，由 `ctool4j-web` 的 `CUnauthorizedExceptionHandler` 统一转为响应体业务码 401（HTTP 状态保持 200，与模块内其余处理器一致）
...
| `CAbstractAuthConfiguration` | auth-spring | 装配基类 | 在 `CAbstractAuthBaseConfiguration` 之上追加默认认证过滤器 bean（`cAuthFilter`）：业务子类实现 `isAuthAnonymous` 即得；业务自建同类型过滤器时不覆盖 |
