...
- **会话服务**：`CAbstractSessionService` 基于 Redis 提供会话存取、按 jwt 取会话、当前会话获取；当前会话缺失（未授权）时 `get()`/`check()` 抛 `CUnauthorizedException`，由 `ctool4j-web` 的 `CUnauthorizedExceptionHandler` 统一转为 HTTP 401（业务码 401）
