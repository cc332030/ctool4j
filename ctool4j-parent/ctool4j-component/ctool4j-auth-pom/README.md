...
| `CSessionUtils` | auth-base | 工具类 | 会话读取静态门面（方法名不带 `Session` 后缀）：`load(request)` 按请求加载、`getDefaultNull()`/`get()` 取当前会话（前者无会话返回 null、后者抛 `CUnauthorizedException`；泛型按调用方声明自动适配） |
