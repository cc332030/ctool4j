...
- **认证过滤器继承链**（自下而上，业务继承链末端）：`CAbstractWebAuthFilter`（`ctool4j-spring-servlet`，类型契约，模块内按档位自动切换容器侧）→ `CAbstractBaseAuthFilter`（auth-base，会话加载与过滤器骨架，不依赖 Spring Security）→ `CAbstractAuthFilter`（auth-spring，Security 认证构造）
