 ...
 *   <li>1.6 无会话：get() 抛出的未授权异常消息契约（get_withoutSession_exceptionMessage）</li>
 * </ul>
 *
 * @since 2026/9/13
 * @version 1.3
...
class CAbstractBaseSessionServiceTests {
    ...
    /**
     * 对应测试用例 1.6：无会话时 get() 抛出的未授权异常消息与 cause 契约
     */
    @Test
    void get_withoutSession_exceptionMessage() {

        // 契约：未授权异常消息固定为「未授权」、不携带 cause（与响应体业务码 401 的判定解耦）
        val service = new SessionServiceStub();

        val e = Assertions.assertThrowsExactly(CUnauthorizedException.class, service::get);
    ...
    private static class SessionServiceStub extends CAbstractBaseSessionService<SessionStub> {
        ...
        /**
         * 扩展点返回值（null 表示无当前会话；用例按需置值，故此处不加 {@code @NonNull}）
         */
        SessionStub session;
