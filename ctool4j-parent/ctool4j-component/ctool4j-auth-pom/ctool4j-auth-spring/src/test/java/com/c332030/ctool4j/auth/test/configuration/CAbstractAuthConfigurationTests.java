...
import com.c332030.ctool4j.auth.configuration.CAbstractAuthConfiguration;
 ...
 * </p>
 * <p>
 * 验证 {@link CAbstractAuthConfiguration} 的唯一逻辑：默认过滤器 bean 把匿名判定委派给业务子类实现的模板方法
 * {@code isAuthAnonymous}。
 * </p>
 *
 ...
 *   与待验证的契约无关。</li>
 *   <li>委派关系用"同一判定的两个分支"证明：判 true / 判 false 各一次，断言过滤器返回值与子类实现一致——
 *   若默认过滤器不再委派（如固定返回 false），用例即失败。</li>
 *   <li>被测类与子类分处两个包，本测试类可编译即证明"业务子类能实现 {@code isAuthAnonymous}"（可见性为
 *   {@code protected}，且不必与本类同包）。</li>
 * </ul>
 * <h2>设计依据</h2>
 * <ul>
 ...
 * <h2>覆盖场景与未覆盖</h2>
 * <ul>
 *   <li>覆盖：默认过滤器对子类匿名判定的委派（两个分支）；跨包子类可编译可用。</li>
 ...
 * <h2>默认过滤器装配</h2>
 * <ul>
 *   <li>1.1 默认过滤器的匿名判定委派给子类实现，两个分支一致（cAuthFilter_isAnonymous_delegatesToSubclass）</li>
 * </ul>
 *
 * @since 2026/9/15
 * @version 1.0
...
class CAbstractAuthConfigurationTests {
    ...
    /**
     * 业务侧装配子类：固定会话类型 + 实现匿名判定（复现业务用法，且与被测类不同包）
     */
    @Configuration
