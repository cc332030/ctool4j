 ...
 *
 * <h2>能力目录</h2>
 * <p>{@code CAuthenticationEntryPoint} 是未认证处理的<b>javax 侧适配器</b>：实现 Spring Security 的
 * {@link AuthenticationEntryPoint}（其签名固定为 javax Servlet 类型），把请求/响应包装成抽象层对象后，
 ...
 *   {@code CDefaultUnauthorizedHandler} 里，本类不做第二份实现——避免 javax/jakarta 两侧逻辑漂移。</li>
 *   <li><b>同名类成对存在</b>：jakarta 侧有同包同名的 {@code CAuthenticationEntryPoint}，
 ...
 * <h2>适用范围</h2>
 * <ul>
 *   <li>javax（Spring Boot 2.x / Servlet 4.0）工程中的未认证统一处理，由 {@code CSecurityConfiguration} 自动装配。</li>
 * </ul>
 *
 * <h2>不适用与边界场景</h2>
 * <ul>
