...
> 组件模块聚合：18 个组件模块的容器，并按层组织（基础层 → Spring 层 → 应用层 → 中间件层 → 配套层）。
...
| Spring 层 | [ctool4j-spring-pom](ctool4j-spring-pom/README.md) | `ctool4j-spring` / `ctool4j-spring-security` / `ctool4j-spring-cloud` |
