 ...
 *
 * <h2>能力目录</h2>
 * <p>{@code ICHandlerInterceptor} 是 {@link CHandlerInterceptor} 在 <b>javax</b> 侧的落地（适配器）：
 * 同时继承 Spring 的 {@code HandlerInterceptor} 与抽象契约 {@link CHandlerInterceptor}，
 ...
 * <h2>设计要点</h2>
 * <ul>
 *   <li><b>多继承接入两侧</b>：{@code extends CHandlerInterceptor, HandlerInterceptor}——Spring 侧负责被容器发现与调用，
 ...
 * <h2>兜底设计</h2>
 * <ul>
 *   <li>无兜底：使用方未覆写任一方法时，走 {@link CHandlerInterceptor} 的默认实现（放行 + 空实现），
 ...
 * <h2>适用范围</h2>
 * <ul>
 *   <li>运行在 javax（Spring Boot 2.x / Servlet 4.0）容器下的拦截器实现。</li>
 * </ul>
 *
 * <h2>不适用与边界场景</h2>
 * <ul>
 ...
 * @version 1.0
 */
public interface ICHandlerInterceptor extends CHandlerInterceptor, HandlerInterceptor {
    ...
    default boolean preHandle(
        ...
        return CHandlerInterceptor.super.preHandle(httpRequest, httpResponse, handler);
    ...
    default void postHandle(
        ...
        CHandlerInterceptor.super.postHandle(httpRequest, httpResponse, handler, modelAndView);
    ...
    default void afterCompletion(
        ...
        CHandlerInterceptor.super.afterCompletion(httpRequest, httpResponse, handler, ex);
