 ...
 *
 * <h2>能力目录</h2>
 * <p>{@code CHttpServletResponse} 是 {@link CHttpResponse} 在 <b>jakarta</b> 侧的落地（适配器）：
 * 持有 {@code jakarta.servlet.http.HttpServletResponse}，把接口声明的公共方法逐个转发给它。</p>
 ...
 *   <li><b>与请求适配器对称</b>：{@link CHttpServletRequest} 与 {@code CHttpServletResponse} 同属 jakarta 侧落地，
 *   创建方式与转发形态一致。</li>
 * </ul>
 *
 * <h2>兜底设计</h2>
 ...
 * <h2>适用范围</h2>
 * <ul>
 *   <li>运行在 jakarta（Spring Boot 3.x / Servlet 5.0+）容器下、需要面向 {@link CHttpResponse} 编写公共代码的场景。</li>
 * </ul>
 *
 * <h2>已知限制与取舍</h2>
 * <ul>
 *   <li>只覆盖两边公共面，jakarta 特有的能力（二进制输出流、Cookie、状态码常量）不在此暴露，需要时直接取底层响应。</li>
...
public final class CHttpServletResponse implements CHttpResponse {
     ...
     * @return 底层 jakarta 响应；入参为 null 时返回 null
