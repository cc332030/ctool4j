 ...
 *
 * <h2>能力目录</h2>
 * <p>{@code CFilterChainAdapter} 是 {@link CFilterChain} 在 <b>jakarta</b> 侧的落地（适配器）：
 * 包装 {@code jakarta.servlet.FilterChain}，把抽象层的放行调用解包成 jakarta 请求/响应后交给底层链。</p>
 ...
 * <h2>适用范围</h2>
 * <ul>
 *   <li>jakarta 容器下 {@link CFilterAdapter} 放行请求，或需要在过滤器之间传递链对象的场景。</li>
 * </ul>
 *
 * <h2>已知限制与取舍</h2>
 * <ul>
...
public final class CFilterChainAdapter implements CFilterChain {
     ...
     * 以 jakarta 过滤器链创建适配器
     *
     * @param chain jakarta 过滤器链，不可为 null
     * @return 适配器实例
    ...
    /**
     * 放行到链上的下一个过滤器：把抽象层请求/响应解包为 jakarta 对象后交给底层链
     *
     * @param request  请求
     * @param response 响应
