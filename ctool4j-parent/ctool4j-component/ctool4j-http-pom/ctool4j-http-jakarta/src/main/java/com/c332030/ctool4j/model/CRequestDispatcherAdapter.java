 ...
 *
 * <h2>能力目录</h2>
 * <p>{@code CRequestDispatcherAdapter} 是 {@link CRequestDispatcher} 在 <b>jakarta</b> 侧的落地（适配器）：
 * 包装 {@code jakarta.servlet.RequestDispatcher}，把抽象层的转发调用解包成 jakarta 请求/响应后交给底层转发器。</p>
 ...
 * <h2>适用范围</h2>
 * <ul>
 *   <li>jakarta 容器下需要在过滤器/拦截器中把请求转发或并入其它资源的场景。</li>
 * </ul>
 *
 * <h2>已知限制与取舍</h2>
 * <ul>
...
public final class CRequestDispatcherAdapter implements CRequestDispatcher {
    ...
    /**
     * 以 jakarta 请求转发器创建适配器
     *
     * @param dispatcher jakarta 请求转发器，不可为 null
     * @return 适配器实例
